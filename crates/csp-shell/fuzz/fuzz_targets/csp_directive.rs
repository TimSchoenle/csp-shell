...
//!
//! The parse itself is fuzzed by the typed crate. What this target covers is the layer above it —
...
fuzz_target!(|input: (u8, Vec<String>)| {
    let (index, texts) = input;
    ...
    let csp = match Csp::new().directive(directive, sources) {
        ...
        Err(CspError::RoutedSourceExpression { source, .. }) => {
            // A refusal must name a source that was actually offered, or the error is pointing the
            // caller at a method for something they did not write.
            assert!(
