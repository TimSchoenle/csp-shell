...
//!
//! Every oracle exposes two entry points. `check(&[u8])` is what the fuzz target calls: it
//! decodes raw bytes the way `libfuzzer-sys` would, so a corpus entry means the same thing in
//! both places. `run(…)` takes the decoded value, which is what lets the replay suite build
//! inputs it can describe rather than byte strings nobody can read.
