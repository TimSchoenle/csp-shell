...
`Csp` covers the directives this crate has an opinion about. Everything else, such as `sandbox`,
`trusted-types`, `webrtc` and the reporting directives, goes through `Csp::set` with the same
vocabulary:
...
onto what it injects. That is why nothing is stamped into the shell. The header is the entire
