//! Shim over [`csp_policy_fuzz::oracle::parse_source`], where the oracle and its documentation
//! live.
//!
...
#[cfg(fuzzing)]
libfuzzer_sys::fuzz_target!(|data: &[u8]| csp_policy_fuzz::oracle::parse_source::check(data));

#[cfg(not(fuzzing))]
fn main() {
    eprintln!(
        "built without --cfg fuzzing; run this through `cargo +nightly fuzz run parse_source` \
