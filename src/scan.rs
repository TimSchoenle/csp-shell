...
pub struct ScanResult {
    /// `'sha256-…'` source expressions, in document order.
    ///
...
/// assert!(scan.hashes[0].starts_with("'sha256-"));
///
...
/// The `'sha256-…'` source expression for one script's text content.
fn sha256_source(content: &str) -> String {
    let digest = Sha256::digest(content.as_bytes());
    let mut source = String::with_capacity(9 + 44 + 1);
    source.push_str("'sha256-");
    source.push_str(&base64::encode(&digest));
    source.push('\'');
    source
}
